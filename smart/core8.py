...
class Engine:
    ...
    async def _start(self):
        ...
        while not self.stop:
            ...
            if len(self.request_generator_queue) > 0:
                print('request_generator_queue', len(self.request_generator_queue))
                for request_or_item in self.iter_request():
                    if request_or_item is None:
                        break
                    print("request_or_item", request_or_item)
    ...
    async def start(self):
        ...
        works = [asyncio.ensure_future(self._start()) for _ in range(2)]
